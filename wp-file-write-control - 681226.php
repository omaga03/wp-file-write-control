...
class WP_File_Write_Control
{
    ...
    public function print_admin_styles()
    {
        ...
        <style>
            * {
            ...
            /* [เพิ่ม CSS ใหม่] จัดระเบียบตัวเลือกใน Card */
            .wfwc-mode-selector {
                background: #f8fafc;
            ...
            .wfwc-radio-item:last-child {
                margin-bottom: 0;
            }

            .wfwc-radio-item input {
                margin-top: 3px;
            }
            ...
            /* Responsive Grid: ปรับตัวอัตโนมัติในช่องแคบ */
            .wfwc-mode-grid {
                display: grid;
                /* ถ้าพื้นที่แคบกว่า 220px จะซ้อนเป็น 2 แถวอัตโนมัติ */
            ...
            .wfwc-radio-compact:hover {
                background: #f1f5f9;
                border-color: #94a3b8;
            }

            /* Highlight สีเมื่อเลือก */
            .wfwc-radio-compact:has(input[value="dir_only"]:checked) {
                background-color: #f0fdf4;
                border: 1px solid #16a34a;
                color: #15803d;
            }

            .wfwc-radio-compact:has(input[value="all_recursive"]:checked) {
                background-color: #fff7ed;
                border: 1px solid #ea580c;
                color: #c2410c;
            }

            .wfwc-radio-compact input {
            ...
            /* [เพิ่ม] สไตล์สำหรับตอนที่ปุ่มถูกล็อค (Disabled) */
            .wfwc-radio-compact:has(input:disabled) {
                cursor: not-allowed !important;
                /* เมาส์เป็นรูปห้ามผ่าน */
                opacity: 0.6;
                /* จางลง */
            ...
            /* แต่ตัวที่ถูกเลือกอยู่ (Checked) ให้ยังพอเห็นสีได้บ้าง (เพื่อบอกว่ากำลังรันโหมดไหน) */
            .wfwc-radio-compact:has(input:disabled:checked) {
                opacity: 0.9;
                filter: none;
                border-style: dashed;
                /* เปลี่ยนขอบเป็นเส้นประ สื่อว่าล็อคอยู่ */
            }
        ...
        <script>
            jQuery(document).ready(function ($) {
                ...
                $(document).on('click', '.wfwc-ajax-toggle', function (e) {
                    ...
                    // [แก้ไขใหม่] ดึงค่า Mode จาก Radio Button ที่ชื่อตรงกับ Type นี้เท่านั้น
                    // เช่น ถ้ากดปุ่ม upload ก็จะไปหา name="wfwc_mode_upload"
                    var inputName = 'wfwc_mode_' + type;
                    var selectedMode = $('input[name="' + inputName + '"]:checked').val();

                    if (!selectedMode) selectedMode = 'dir_only'; // กันเหนียว
    ...
    public function ajax_toggle_upload()
    {
        ...
        $req_mode = sanitize_key($_POST['mode'] ?? 'dir_only'); // ค่าที่ส่งมาจากหน้าจอ
        ...
        // ... (ส่วน Verify และ Return JSON เหมือนเดิม) ...
        usleep(500000);
        if (isset($dirs[0]))
    ...
    private function control_ui()
    {
        ...
        foreach (['upload', 'plugin', 'theme'] as $type) {
            ...
            $current_mode = $info['is_open'] ? $saved_mode : 'dir_only';

            // [ใหม่] ถ้าเปิดอยู่ ให้ล็อคปุ่ม (disabled)
            ...
            <div id="wfwc-card-<?= $type ?>" class="wfwc-card <?= $info['is_open'] ? 'active' : 'inactive' ?>">
                ...
                <?php if ($settings['enable_mode_selector']): ?>
                    <div class="wfwc-mode-grid">
                        <label class="wfwc-radio-compact" title="🚀 Turbo">
                            <input type="radio" name="wfwc_mode_<?= $type ?>" value="dir_only" <?php checked($current_mode, 'dir_only'); ?>                 <?= $disabled_attr ?>>
                            <span class="wfwc-label-text" style="color:#059669;">🚀 Turbo</span>
                        </label>

                        <label class="wfwc-radio-compact" title="🐢 Deep">
                            <input type="radio" name="wfwc_mode_<?= $type ?>" value="all_recursive" <?php checked($current_mode, 'all_recursive'); ?>                 <?= $disabled_attr ?>>
                            <span class="wfwc-label-text" style="color:#d97706;">🐢 Deep</span>
                        </label>
                    </div>
                <?php endif; ?>
                        ...
                        <div class="wfwc-form-group"
                            ...
                            <div class="wfwc-form-header">
                                <label class="wfwc-switch">
                                    <input type="checkbox" name="wfwc_enable_mode_selector" value="1" <?php checked($settings['enable_mode_selector'], 1); ?>>
                                    <span class="slider"></span>
                                </label>
                                <label style="font-weight:bold; color:#334155;">Show Advanced Mode Selector (Turbo /
                                    Deep)</label>
                            </div>
                            <div style="font-size:12px; color:#64748b; margin-left:46px;">
                                <strong>ON:</strong> แสดงปุ่มเลือกโหมด Turbo/Deep ให้ผู้ใช้เลือกได้ตามต้องการ<br>
                                <strong>OFF:</strong> ซ่อนปุ่มเลือกโหมด (Simple View) และบังคับใช้ <strong>🚀 Turbo
                                    Mode</strong> เป็นค่าพื้นฐานเสมอ
                            </div>
                        </div>
    ...
    private function get_settings()
    {
        $defaults = [
            ...
            'api_key' => '',
            'allowed_ips' => '',
            'enable_mode_selector' => 1 // [เพิ่ม] ค่าเริ่มต้น: 1 = เปิดโชว์ปุ่ม, 0 = ซ่อน
        ];
    ...
    public function handle_save_settings()
    {
        ...
        $input = [
            // [ใหม่] รับค่า Settings
            'enable_mode_selector' => isset($_POST['wfwc_enable_mode_selector']) ? 1 : 0,
    ...
    public function render_admin_notices()
    {
        global $pagenow;
        $t = '';
        if ($pagenow === 'upload.php') $t = 'upload';
        elseif ($pagenow === 'plugins.php') $t = 'plugin';
        elseif ($pagenow === 'themes.php') $t = 'theme';
            ...
            if ($info['exists']) {
                ...
                echo "<div id='wfwc-notice-bar' class='notice' style='background:{$info['bg']}; border-left: 5px solid {$info['color']}; display:flex; align-items:center; justify-content:space-between; padding:10px 20px; margin: 20px 0; box-shadow: 0 1px 1px rgba(0,0,0,.04); transition:0.3s;'>
                    ...
                    <div style='display:flex; align-items:center; gap:10px;'>
                        " . ($settings['enable_mode_selector'] ? "
                        <div style='display:flex; gap:5px;'>
                            <label class='wfwc-radio-compact' title='🚀 Turbo' style='height:28px; padding:0 8px; background:#fff;'>
                                <input type='radio' name='wfwc_mode_{$t}' value='dir_only' " . checked($current_mode, 'dir_only', false) . " $disabled_attr>
                                <span class='wfwc-label-text' style='color:#059669; margin-left:4px;'>🚀 Turbo</span>
                            </label>

                            <label class='wfwc-radio-compact' title='🐢 Deep' style='height:28px; padding:0 8px; background:#fff;'>
                                <input type='radio' name='wfwc_mode_{$t}' value='all_recursive' " . checked($current_mode, 'all_recursive', false) . " $disabled_attr>
                                <span class='wfwc-label-text' style='color:#d97706; margin-left:4px;'>🐢 Deep</span>
                            </label>
                        </div>
                        " : "") . "
    ...
    public function render_meta_box($post)
    {
        ...
        if (!$info['exists']) { echo "Error"; return; }
        ...
        if ($settings['enable_mode_selector']) {
            echo "<div class='wfwc-mode-grid' style='margin-bottom:10px;'>
                    <label class='wfwc-radio-compact' title='🚀 Turbo' style='height:30px; justify-content:flex-start; padding-left:8px;'>
                        <input type='radio' name='wfwc_mode_upload' value='dir_only' " . checked($current_mode, 'dir_only', false) . " $disabled_attr>
                        <span class='wfwc-label-text' style='color:#059669;'>🚀 Turbo</span>
                    </label>
                    <label class='wfwc-radio-compact' title='🐢 Deep' style='height:30px; justify-content:flex-start; padding-left:8px;'>
                        <input type='radio' name='wfwc_mode_upload' value='all_recursive' " . checked($current_mode, 'all_recursive', false) . " $disabled_attr>
                        <span class='wfwc-label-text' style='color:#d97706;'>🐢 Deep</span>
                    </label>
                  </div>";
        }
